...
export async function sendRequestToJoin(terminId, autoApprove) {
  ...
  if (autoApprove) {
    const { error } = await supabase.rpc('join_termin', { termin_id: terminId });
    if (error) return { success: false, message: error.message };
  ...
  const { error } = await supabase.from('termin_requests').upsert(
    {
      termin_id: terminId,
      user_id: user.id,
      status: 'pending',
...
export async function leaveJoinedTermin(terminId) {
  ...
  const { data: termin } = await supabase.from('termins').select('creator_id, title, playground').eq('id', terminId).single();

  const { error } = await supabase.rpc('leave_termin', { termin_id: terminId });
...
export async function deleteTermin(terminId) {
  ...
  const { data: termin } = await supabase.from('termins').select('creator_id').eq('id', terminId).single();
  if (!termin || termin.creator_id !== user?.id) return { success: false };
...
export async function approveJoinRequest(requestId, terminId, userId) {
  ...
  const { error } = await supabase.from('termin_requests').delete().eq('id', requestId);
...
export async function assignPlayerToTeam(terminId, userId, team) {
  const { data: termin } = await supabase.from('termins').select('teams, registered_players, result_entered_at, max_players, event_date, event_time').eq('id', terminId).single();
  ...
  if (termin.event_date) {
    ...
    if (new Date() > cutoff) {
      return { success: false, message: 'Ekipe se mogu mijenjati najkasnije 6 sati nakon početka termina.' };
    }
  }
